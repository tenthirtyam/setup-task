 ...
 */
function getTaskExecutableName() {
...
async function restoreCache(version) {
    ...
    try {
        ...
        const hitKey = await cache.restoreCache([cacheDir], cacheKey);
        if (!hitKey) {
            core.debug(`Task ${version} not found in cache`);
            return '';
        }
        core.info(`Task ${version} found in cache`);
        const taskPath = path.join(cacheDir, executableName);
...
async function saveCache(taskPath, version) {
    ...
    try {
        ...
        const destPath = path.join(cacheDir, executableName);
